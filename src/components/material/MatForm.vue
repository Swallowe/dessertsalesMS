...
<script>
    ...
    export default {
        ...
        methods: {
            getFlow() {
                this.$axios.post("/api/materialFlow/findAllMaterialFlow").then(res =>{
            ...
            onFindById(formName) {
                if (va == 1) {
                    this.$axios.post("/api/materialFlow/findMaterialFlowById", this.mymodelid).then(res => {
                        this.tableData = res.data.data;
            ...
            onFindByType(formName) {
                if (va == 2) {
                    this.$axios.post("/api/materialFlow/findMaterialFlowByType", this.mymodeltype).then(res => {
                        this.tableData = res.data.data;
                        if (res.data.data != null) {
                            this.$message("查询成功")
                        } else {
